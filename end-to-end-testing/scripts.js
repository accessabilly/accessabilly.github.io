...
function toggleButtonClickHandler (event) {
  if (
    event.currentTarget.tagName === 'button' || event.currentTarget.tagName === 'BUTTON'
  ) {
...
function toggleButtonState (button) {
  ...
  button.innerText = isAriaPressed ? 'Off' : 'On';
  var home = document.getElementById('home');
  var search = document.getElementById('search');
  if (isAriaPressed) {
    home.removeAttribute('accesskey');
    search.removeAttribute('accesskey');
  } else {
    home.setAttribute('accesskey', '0');
    search.setAttribute('accesskey', '1');
  }
}
